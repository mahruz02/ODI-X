...
import { DIMENSIONS, TRIANGULATION_PLAN, TRIANGULATION_SOURCES } from "@/lib/questionnaire";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Peta Triangulasi Data — ODI-X" },
      {
        ...
        content: "Kelengkapan data per domain dan metode independen.",
      },
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 animate-fade-in">
        <header className="mb-6">
          ...
          <p className="mt-2 max-w-3xl text-xs text-muted-foreground leading-relaxed">
            ...
            <strong className="text-foreground">2 titik data independen</strong> (kuesioner
            multi-level, FGD, wawancara, atau data objektif/dokumen). Perbedaan pandangan antar
            sumber ditandai sebagai gap, bukan sekadar dirata-ratakan.
          </p>
        </header>
        ...
        <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-2">
            <span className="inline-block size-3 rounded-sm cell-filled" /> Data Tersedia (angka =
            sampel/dokumen)
          </span>
          <span className="inline-flex items-center gap-2">
            <span className="inline-block size-3 rounded-sm cell-empty" /> Direncanakan, Belum Ada
            Data
          </span>
          <span>
...
function Cell({ planned, n }: { planned: "utama" | "pendukung" | null | undefined; n: number }) {
  if (!planned) {
