...
import {
  IndexCard as AssessmentIndexCard,
  RespondentLinkManager as AssessmentRespondentLinkManager,
  ResponseManager as AssessmentResponseManager,
} from "@/components/assessment/AssessmentManagers";
import {
  addRespondentLink,
  getDashboardData,
  getQualitativeData,
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Dashboard Diagnosis Organisasi — ODI-X" },
      {
        ...
        content: "Dashboard interaktif 12 domain kesehatan organisasi.",
      },
...
function DashboardPage() {
  ...
  const respondentLinks: RespondentLink[] = [];

  const summaries = useMemo(() => buildDimensionSummaries(scores), [scores]);
  ...
  const reportReady =
    totalRespondents >= 5 && (totalFgd > 0 || totalInterviews > 0 || totalDocs > 0);
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 animate-fade-in">
        ...
        <div className="mb-6 rounded-2xl border bg-card p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <Link to="/asesmen" className="text-xs font-semibold text-primary hover:underline">
              ...
              <p className="mt-1.5 text-xs text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {data.organization?.sector || "LKMS / BMT"}
                </span>{" "}
                · Responden:{" "}
                <span className="font-semibold text-foreground">{totalRespondents} orang</span> ·
                Tanggal: {data.organization?.started_on}
              </p>
            </div>
        ...
        <AssessmentRespondentLinkManager id={id} />
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm">
          <div className="flex items-center gap-3">
            <span
              className={`flex size-3 rounded-full ${reportReady ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`}
            />
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Kuantitatif: {totalRespondents} responden · FGD: {totalFgd} sesi · Wawancara:{" "}
                {totalInterviews} · Dokumen: {totalDocs} terupload
              </p>
            </div>
          </div>
          <Link
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-sm">
            <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2 border-b pb-3">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Perbandingan persepsi antar level manajemen
                </p>
              </div>
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-sm flex flex-col justify-between">
            <div className="mb-3 flex items-baseline justify-between border-b pb-3">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Pencapaian skor rata-rata Likert per domain
                </p>
              </div>
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-accent/40 text-accent-foreground border">
                Target L4
              </span>
            </div>
        ...
        <section className="mb-8 rounded-2xl border bg-card p-5 shadow-sm">
          <div className="mb-4 border-b pb-3">
            ...
            <p className="text-xs text-muted-foreground">
              Warna sel mengindikasikan deviasi skor persepsi. Gap tinggi ($\Delta \ge 1.20$)
              ditandai dengan warna merah untuk memicu investigasi kualitatif.
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs min-w-[640px]">
              <thead className="bg-muted/60 uppercase font-bold text-muted-foreground">
                <tr>
                  <th className="sticky left-0 bg-muted z-10 px-3 py-2.5 text-left shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                    Domain Organisasi
                  </th>
                  {ROLES.map((r) => (
        ...
        <div className="mb-10 grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-sm">
            <div className="mb-3 border-b pb-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                Dihitung komposit: $P_d = (Urgent \times 0.35) + (Impact \times 0.30) + (Risk \times
                0.20) - (Penalty \times 0.15)$
              </p>
            </div>
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-sm">
            <div className="mb-3 border-b pb-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                Klik domain untuk melihat bukti kualitatif detail
              </p>
            </div>
              ...
              {summaries.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${totalProof >= 2 ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/30 border" : "bg-amber-500/10 text-amber-600 border-amber-500/30 border"}`}
                    >
          ...
          <section className="mb-10">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {flaggedComments.map((c, i) => (
                <blockquote key={i} className="rounded-2xl border bg-card p-4 text-sm shadow-sm">
        ...
        {/* Response Manager */}
        <AssessmentResponseManager id={id} />
