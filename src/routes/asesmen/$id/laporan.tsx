...
import { Download, Printer, ShieldCheck, Activity, Target, Layers, LayoutGrid, List } from "lucide-react";
...
function ReportPage() {
  ...
  const [audience, setAudience] = useState<"lengkap" | "eksekutif" | "kepemimpinan" | "sdm" | "risiko">("lengkap");
  const [shareCopied, setShareCopied] = useState(false);
  const [viewMode, setViewMode] = useState<"kanban" | "list">("kanban");

  const filteredRecommendations = useMemo(() => {
    if (audience === "eksekutif") return recommendations.filter((r) => r.priority === "mendesak");
    if (audience === "sdm") return recommendations.filter((r) => [4, 5, 6, 7, 10].includes(r.dimension));
    if (audience === "risiko") return recommendations.filter((r) => [3, 9, 11].includes(r.dimension));
    if (audience === "kepemimpinan") return recommendations.filter((r) => [1, 2, 3, 5, 12].includes(r.dimension));
  ...
  return (
    <ProtectedRoute allowedRoles={["admin", "hr"]}>
      ...
      <div className="no-print mb-8 flex flex-wrap items-center justify-between gap-3 border-b pb-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {shareCopied ? "Link Read-Only (30 Hari) Tersalin!" : "Bagikan Laporan Read-Only (30 Hari)"}
          </button>
          <button
            type="button"
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Disusun {today} · Sample: {respondents} responden · Triangulasi Persepsi 4 Prespektif, Data Kuantitatif & Dokumen Pendukung.
            </p>
          </div>
        </div>
      </header>
      ...
      <section className="mb-10">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-4">
          <div className="rounded-2xl border bg-card p-4 shadow-sm">
            ...
            <p className="text-2xl font-extrabold text-primary tabular-nums mt-1">{indices.healthIndex}/100</p>
            <p className="text-[10px] text-muted-foreground mt-1">Skor kesehatan agregat 12 domain</p>
          </div>
          <div className="rounded-2xl border bg-card p-4 shadow-sm">
            <span className="text-xs text-muted-foreground font-semibold">Alignment Index</span>
            <p className="text-2xl font-extrabold text-foreground tabular-nums mt-1">{indices.alignmentIndex}/100</p>
          ...
          <div className="rounded-2xl border bg-card p-4 shadow-sm">
            ...
            <p className="text-xl font-extrabold text-foreground tabular-nums mt-1">{indices.maturityLabel}</p>
            <p className="text-[10px] text-muted-foreground mt-1">Tingkat kematangan organisasi</p>
          </div>
          <div className="rounded-2xl border bg-card p-4 shadow-sm">
            <span className="text-xs text-muted-foreground font-semibold">Risk Exposure</span>
            <p className="text-2xl font-extrabold text-destructive tabular-nums mt-1">{indices.riskExposure}</p>
            <p className="text-[10px] text-muted-foreground mt-1">Status paparan risiko internal</p>
          </div>
        </div>

        <div className="rounded-2xl border bg-card p-5 text-xs leading-relaxed shadow-sm print:shadow-none">
          <p>
            Berdasarkan pengolahan data kuesioner diagnosis 12 domain, kesehatan organisasi <strong>{orgName}</strong> berada pada tingkat kematangan <strong>{indices.maturityLabel}</strong> dengan <strong>Health Index {indices.healthIndex}/100</strong> dan <strong>Alignment Index {indices.alignmentIndex}/100</strong>.
          </p>
          <p className="mt-2">
            Readiness for Change score tercatat <strong>{indices.readinessScore}/100</strong>. Temuan mengindikasikan perlunya tindak lanjut cepat pada domain dengan gap persepsi antar level yang lebar guna menyelaraskan ekspektasi sebelum rekomendasi operasional dijalankan.
          </p>
        </div>
      </section>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b pb-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              Klasifikasi arah rekomendasi: Pertahankan, Perbaiki, Bangun, Transformasi, Kurangi, Hentikan, Eksplorasi.
            </p>
          </div>
          ...
          <div className="no-print flex items-center gap-1.5 rounded-xl border bg-card p-1 text-xs font-semibold">
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 transition-all ${
                viewMode === "kanban" ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted text-muted-foreground"
              }`}
            >
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <div className="mt-3 grid gap-2 text-xs sm:grid-cols-4 bg-muted/30 p-3 rounded-xl">
                <div>
                  <span className="text-muted-foreground font-medium block text-[10px]">Horizon Waktu:</span>
                  <span className="font-bold text-foreground">{r.horizon}</span>
                </div>
                <div>
                  <span className="text-muted-foreground font-medium block text-[10px]">Penanggung Jawab (PIC):</span>
                  <span className="font-bold text-foreground">{r.pic}</span>
                </div>
                <div>
                  <span className="text-muted-foreground font-medium block text-[10px]">KPI Target:</span>
                  <span className="font-bold text-foreground">{r.kpi}</span>
                </div>
                <div>
                  <span className="text-muted-foreground font-medium block text-[10px]">Target Capaian:</span>
              ...
              <div className="mt-3 border-t pt-2">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Tindakan Rekomendasi Utama:</span>
                ...
                <tbody className="divide-y">
                  {qual.metrics.map((m) => (
                    <tr key={m.id}>
                      ...
                      <td className="px-3 py-2 text-center tabular-nums">{m.target_val || "—"}</td>
                      <td className="px-3 py-2 text-center font-bold tabular-nums">{m.actual_val || "—"}</td>
                      <td className="px-3 py-2 text-center">{m.period || "—"}</td>
                      <td className="px-3 py-2 text-center font-semibold capitalize">{m.confidence_level}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {interviewsWithFindings.slice(0, 4).map((iv) => (
            <div key={iv.id} className="rounded-xl border bg-card p-3 text-xs">
              <span className="font-bold text-primary block">Wawancara ({iv.informant_role})</span>
      ...
      <footer className="border-t pt-4 text-[11px] text-muted-foreground text-center">
        Laporan hasil diagnosis ini diproses otomatis oleh sistem ODI-X pada {today}. Angka dan rekomendasi terhubung langsung dengan bukti kualitatif dan data kuantitatif pendukung.
      </footer>
    </main>
    </ProtectedRoute>
  );
