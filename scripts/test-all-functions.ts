import { DIMENSIONS, ROLE_LABELS, dimensionsForRole } from "../src/lib/questionnaire";
import {
  buildDimensionSummaries,
  computeCompositeIndices,
  buildRecommendations,
} from "../src/lib/report";
import { buildEvidenceMap, computeConfidence, evidenceFor } from "../src/lib/confidence";
import { buildPriorities, topPriorities } from "../src/lib/priority";
...
for (const role of ["pengurus", "manajemen", "karyawan", "stakeholder"] as const) {
...
assert(
  Number(summaries[0].gap.toFixed(1)) === 1.2,
  "Hitung deviasi gap persepsi max - min (4.2 - 3.0 = 1.2)",
);
...
assert(
  indices.alignmentIndex > 0 && indices.alignmentIndex <= 100,
  "Alignment Index dalam skala 0..100",
);
assert(
  indices.adaptabilityIndex > 0 && indices.adaptabilityIndex <= 100,
  "Adaptability Index dalam skala 0..100",
);
assert(
  indices.maturityLevel >= 1 && indices.maturityLevel <= 5,
  "Maturity Level bernilai 1 s.d. 5",
);
...
assert(
  ev1.fgd === 1 && ev1.interviews === 1 && ev1.documents === 1,
  "Evidence map menghitung bukti kualitatif domain 1",
);

const conf1 = computeConfidence(ev1);
assert(
  conf1.level === "kuat" && conf1.methods === 4,
  "3 sumber kualitatif + kuantitatif = Confidence Level 'kuat'",
);

// 4. Tes Priority Engine
const priorities = buildPriorities(summaries, (dimId) =>
  computeConfidence(evidenceFor(mockEvidenceMap, dimId)),
);
...
assert(
  recs[0].horizon && recs[0].actions.length > 0,
  "Rekomendasi memiliki horizon dan tindakan aksi",
);
